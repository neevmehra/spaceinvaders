...
// I.e., PortB has already been reset and activated (do not reset PortB here again)
    ...
    IOMUX->SECCFG.PINCM[PB0INDEX] = (uint32_t) 0x00000081; // GPIOB Pin 0
    IOMUX->SECCFG.PINCM[PB1INDEX] = (uint32_t) 0x00000081; // GPIOB Pin 1
    IOMUX->SECCFG.PINCM[PB2INDEX] = (uint32_t) 0x00000081; // GPIOB Pin 2
    IOMUX->SECCFG.PINCM[PB3INDEX] = (uint32_t) 0x00000081; // GPIOB Pin 3
    IOMUX->SECCFG.PINCM[PB4INDEX] = (uint32_t) 0x00000081; // GPIOB Pin 4

    GPIOB->DOE31_0 |= (uint32_t) 0x01F; // Enable the outputs
}
...
void DAC5_Out(uint32_t data){
    // write this
    //uint32_t input = (data * 3.3)/31;
    GPIOB->DOUT31_0 = ((GPIOB->DOUT31_0 & ~(0x01F)) | data);
